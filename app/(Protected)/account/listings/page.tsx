...
// Import Data
import { GENERAL_SETTINGS, LISTINGS_SETTINGS } from "@/constants";
// Import Assets & Icons
import { PlusIcon } from "lucide-react";

export const metadata: Metadata = {
  title: `${LISTINGS_SETTINGS.pluralName} Overview`,
};
    ...
    <SectionOuterContainer className="grid flex-1 items-start gap-4 px-6 sm:px-0 py-6">
      <div className="flex justify-end space-x-2">
        ...
        >
          ...
          New {LISTINGS_SETTINGS.singularName}
        </Link>
      </div>
      <Card>
        <CardHeader>
          <SectionTitle className="mx-0 max-w-none text-left">
            Your {LISTINGS_SETTINGS.pluralName}
          </SectionTitle>
          ...
          {listingData.length === 0 ? (
            <div className="flex flex-1 items-center justify-center rounded-lg border border-dashed shadow-sm min-h-96">
              <div className="flex flex-col items-center gap-1 text-center">
                ...
                >
                  Add {LISTINGS_SETTINGS.singularName}
                </Link>
              </div>
            </div>
          ) : (
