        ...
        <Disclosure.Panel className="mt-10 grid sm:grid-cols-[3fr_3fr_3fr_1.5fr] gap-8">
          {steps.map(({ name, href, imageSrc, imageAlt, buttonText }) => (
            <div key={name} className="group">
              ...
              <h3 className="mt-4 text-base font-semibold text-primaryDz">
                {name}
              </h3>
              <Link
                href={href}
                className="mt-2 block w-full border-2 border-b-[3px] border-primaryDz bg-white px-4 py-2 text-center text-sm font-medium text-primaryDz hover:bg-gray-50"
              >
                {buttonText}
              </Link>
            </div>
          ...
          <div className="flex flex-col items-center justify-center bg-primaryDz text-center text-white p-6">
            <h3 className="text-xl font-bold mb-4">Sign up to get</h3>
            <Link
              href="#"
              className="w-full bg-white px-4 py-2 text-sm font-medium text-primaryDz hover:bg-gray-100"
            >
              Join Now
            </Link>
          </div>
